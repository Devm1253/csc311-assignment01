...
public enum FuelType {
    ...
    public static FuelType fromLabel(String text) {
        if (GASOLINE.label.equalsIgnoreCase(text.trim())) {
            return GASOLINE;
        } else if (DIESEL.label.equalsIgnoreCase(text.trim())) {
            return DIESEL;
        } else if (ELECTRIC.label.equalsIgnoreCase(text.trim())) {
            return ELECTRIC;
        } else if (HYBRID.label.equalsIgnoreCase(text.trim())) {
            return HYBRID;
        } else {
            throw new IllegalArgumentException("No FuelType with label: " + text);
        }
    } }